...
class UploadsMiddleware(AgentMiddleware[UploadsMiddlewareState]):
    ...
    def _create_files_message(self, new_files: list[dict], historical_files: list[dict]) -> str:
        ...
        lines.append("")
        lines.append("**For spreadsheets (.xlsx / .xlsm / .csv): read the original file directly with")
        lines.append("openpyxl / pandas / DuckDB — DO NOT `read_file` the `.docling.json` sibling.**")
        lines.append("Spreadsheets are already structured grids; docling wraps every cell in 13 fields")
        lines.append("of layout metadata (row_span, col_span, offset_idx, etc.) and bloats the file ~50×")
        lines.append("with information you don't need. The `.docling.summary.json` block above already")
        lines.append("tells you sheet names + row/col counts, which is everything you need to plan the")
        lines.append("openpyxl read. Recommended preludes:")
        lines.append("  - `openpyxl.load_workbook(path, data_only=True, read_only=True)` for ≤100K cells")
        lines.append("  - `pd.read_excel(path, sheet_name=…)` if you need a DataFrame")
        lines.append("  - DuckDB `read_xlsx(path, sheet=, range=)` for huge or selective reads")
        lines.append("")
        lines.append("For huge .docx / .pptx, stream via `zipfile` + `lxml.etree.iterparse` rather than")
        lines.append("loading into context. The `.docling.json` is best for PDF / Word where layout")
        lines.append("(headings, tables across pages, embedded images) actually carries information.")
