...
- Extends `AgentState` with: `sandbox`, `thread_data`, `title`, `artifacts`, `todos`, `uploaded_files`, `viewed_images`, `selected_template`
- Uses custom reducers: `merge_artifacts` (deduplicate), `merge_viewed_images` (merge/clear)
- `selected_template` carries `{template_id, name, type, jinja_download_url, fields}` injected by the gateway when chat_stream sees a non-null `selected_template` in the request body. Consumed by `fill_template` tool.
...
| **Templates** (`/api/template`) | `POST /extract_fields` - multipart upload, runs `src/skills/template_filler/extractor.extract_fields` against an uploaded .docx/.xlsx and returns the AI-suggested field schema. Called by trademind-backend/services/user during template upload. |
...
3. **Built-in tools**:
   ...
   - `fill_template` - Render the user-selected template (`ThreadState.selected_template`) with AI-supplied data via docxtpl/xltpl, write to thread outputs, mirror to OSS chat bucket, emit artifact metadata. Tool no-ops with an Error message if no template is selected — agent falls back to plain output via `present_files`.
...
DeerFlow exposes vision via a **two-path split**: the lead agent stays on a text-only thinking model (`deepseek-v4-pro` in trademind) so its reasoning chain is preserved, and image work is dispatched to either a tool or a subagent depending on whether the image is a structured trade document:

- **Trade documents** → `extract_trade_document` builtin tool (`src/tools/builtins/extract_trade_document_tool.py`, model `qwen-vl-ocr-latest`). Single LLM call: tool reads the image, base64-encodes it, sends one `HumanMessage` with `[image_url, text]` content blocks directly to qwen-vl-ocr, parses the returned JSON against `OcrEnvelope`, writes `/mnt/user-data/outputs/<doc_type>_ocr.json`, mirrors to OSS, updates `artifacts` + `artifact_metadata` state. No subagent, no agent loop, no `present_files` follow-up needed. The `qwen-vl-ocr-latest` checkpoint is single-turn — it rejects system prompts, multi-turn history, and tool-call sequences (400 "messages do not contain elements with the role of user"), so it can NOT be wired into a `create_agent` flow. As a tool we control the messages payload directly, which is the only shape the model accepts AND the cheapest path (qwen-vl-ocr is ~5-10× cheaper output than qwen-vl-plus / qwen-vl-max).
- **Free-form image Q&A** → `vision-analyst` subagent (`src/subagents/builtins/vision_analyst.py`, model `qwen-vl-plus-latest`). Multi-turn agent loop with `view_image_tool` + `ViewImageMiddleware`, returns natural-language observations only. Used for product photos / factory shots / screenshots where the user wants commentary rather than fields.
